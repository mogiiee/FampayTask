...
def YoutubeCaller(number, insert_query):
    try:
        service = build('youtube', 'v3', developerKey= exporter.API_KEY_1) #put in your own api key from the .env file
    except:
        service = build('youtube', 'v3', developerKey= exporter.API_KEY_2) #put in your own api key2 from the .env file
    #api_key2 will only kick in when api_key 1 fails
    timern = time_manager.get_yt_datetime()
    req = service.search().list(part = 'snippet', q = insert_query, maxResults= number, type='video', order= 'date', publishedAfter= timern)
